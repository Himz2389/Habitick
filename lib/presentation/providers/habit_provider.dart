...
class HabitNotifier extends StateNotifier<List<HabitModel>> {
  ...
  Future<void> updateHabit(HabitModel updatedHabit) async {
    ...
    final oldHabit = state.firstWhere((h) => h.id == updatedHabit.id, orElse: () => updatedHabit);
    ...
    if (finalHabitToSave.isCompleted == 1 || finalHabitToSave.isDeleted == 1 || finalHabitToSave.isPaused == 1) {
  ...
  Future<void> syncCategoryColor(String categoryId, String newColor) async {
    ...
    final habitsToUpdate = state.where((h) => h.categoryId == categoryId).toList();
